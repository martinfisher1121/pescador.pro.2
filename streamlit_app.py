...
if st.session_state.end_time:
    while st.session_state.end_time > datetime.now():
        ...
        h, m = divmod(m, 60)
        timer_placeholder.header(f"⏳ {h:02d}:{m:02d}:{secs:02d}" if 'secs' in locals() else f"⏳ {h:02d}:{m:02d}:{s:02d}")
        time.sleep(1)
        st.rerun()
